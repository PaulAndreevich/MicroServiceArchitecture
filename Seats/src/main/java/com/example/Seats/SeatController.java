...
public class SeatController {
    ...
    SeatController(SeatRepository repository){
        this.repository = repository;
    }

    @GetMapping(path = "/seats",produces = MediaType.APPLICATION_JSON_VALUE)
    public Resources<Resource<Seat>> all(){
        List<Resource<Seat>> Seats = repository.findAll().stream()
                .map(employee -> new Resource<>(employee,
                        linkTo(methodOn(SeatController.class).one(employee.getId())).withSelfRel(),
