...
public class GatewayController {
    ...
    void deleteOrder(@PathVariable Long reservationid) throws IOException, JSONException {
        List<Long> temp = getSeatsFromReservation(reservationid);

        for (Long i: temp) {
            //System.out.println("HERE I AM");
            URL url = new URL("http://localhost:8060/seats/" + i + "/false");
    ...
    @PostMapping(path = "/reservation/{id}/add/{seat}", produces = MediaType.APPLICATION_JSON_VALUE)
    //добавить место к заказу
    public String addSeatToReservation(@PathVariable Long id, @PathVariable Long seat) throws IOException, JSONException{

        // Получить значение: Занято ли это место?

        System.out.println(seat);

        String url = "http://localhost:8060/seats/" + seat;
        ...
        Boolean isSeatTaken = jsonObject.getBoolean("taken");

        //System.out.println("IS THE SEAT TAKEN? >>>> " + isSeatTaken);

        if (isSeatTaken == false){
            //System.out.println("HERE I AM");
            website = new URL("http://localhost:8060/seats/" + seat + "/true");
            ...
            website = new URL("http://localhost:8090/reservations/" + id + "/seat/" + seat);
    ...
    private List<Long> getSeatsFromReservation(Long reservationID) throws IOException, JSONException{
        ...
        JSONArray jsonArray = jsonObject.getJSONArray("seatsIDs");

        System.out.println("##### HEREEEE:    [" + response.toString() + "]   <<< HEREEE");

        for (int i = 0; i < jsonArray.length(); i++) {

            seats.add(jsonArray.getLong(i));
